// pages/UserAnswersReviewPage.jsx
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        if (isMounted) setError(err?.response?.data?.error ?? "Failed to load pending answers");
      } finally {
    ...
    return data.references
      .map((r) => ({
        ...
        answers: r.answers.filter((a) => {
          const matchSearch   = !term || a.question_text.toLowerCase().includes(term) || (a.comment ?? "").toLowerCase().includes(term);
          const matchResponse = responseFilter === "ALL" || a.response === responseFilter;
  ...
  const handleAnswerDecided = useCallback((answerId) => {
    setData((prev) => {
      ...
      const references = prev.references
        .map((r) => ({ ...r, answers: r.answers.filter((a) => a.answer_id !== answerId) }))
        .filter((r) => r.answers.length > 0);
      return { ...prev, references, total_pending: Math.max(0, (prev.total_pending ?? 0) - 1) };
    });
  }, []);
      ...
      <div className="flex h-screen bg-gray-50 font-sans">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <main className="flex-1 flex items-center justify-center">
            <p className="text-sm text-red-500">User ID is missing from the URL.</p>
          </main>
        </div>
      </div>
    );
    ...
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-8 py-8">
          <Link to="/admin/users" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-4">
            <ArrowLeft className="w-4 h-4" />
            Back to Users
          </Link>

          {loading && <p className="text-sm text-gray-400">Loading pending answers…</p>}
            ...
            <>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      ...
                      <span className="text-xs text-gray-400">Joined {formatDate(data.user.joined_at)}</span>
                      <Link to={`/admin/users/${userId}/submissions`} className="text-xs text-blue-700 hover:underline">
              ...
              <div className="flex flex-wrap items-center gap-3 mb-4">
                ...
                {["ALL", "YES", "NO"].map((r) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium border ${responseFilter === r ? "bg-blue-900 text-white border-blue-900" : "bg-white text-gray-600 border-gray-200"}`}
                  >
              ...
              {visibleReferences.length === 0 ? (
                ...
                visibleReferences.map((ref) => (
                  <section key={ref.reference_id} className="bg-white border rounded-2xl mb-6">
                    <header className="px-6 py-4 border-b flex justify-between items-center">
                      <h2 className="font-semibold">{ref.reference_title}</h2>
                      <span className="text-xs text-gray-400">{ref.answers.length} pending</span>
                    </header>
                    {ref.answers.map((a) => (
                      <AnswerReviewCard
                        key={a.answer_id}
                        answer={a}
                        readOnly={readOnly}
                        onDecided={readOnly ? undefined : handleAnswerDecided}
                      />
                    ))}
                  </section>
                ))
